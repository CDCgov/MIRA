## Running MIRA with Illumina Data
...
* Copy the run's demultiplexed fastqs into the RUN-FOLDER.
    * Create another folder inside your RUN-FOLDER called "fastqs" and copy the folder containing demultiplexed fastqs into this newly created fastqs folder so that the folder structure is FLU_SC2_SEQUENCING/RUN-FOLDER/fastqs/demultiplexed-fastqs/sample-reads.fastq. There will be two fastq files per sample.
...
* Click the `REFRESH RUN LISTING` button and select your run from  the dropdown box.
    ...
    * Next, click the box that say "What kind of data is this" and select Flu-Illumina or SC2-Whole-Genome-Illumina

    ![](images/mira_whatkindofdata.png)

* If you select SC2-Whole-Genome-Illumina, a new box will appear for you to select your primer schema/amplicon approach 
    * _NOTE: MIRA Whole Genome Illumina module's primer trimming has been tested and optimized for xGen primer panel. We cannot guarantee perfect trimming for other panels._
...
* Your samplenames will be inferred from the fastq filenames in your fastq directory. **Do not change these sample names**
...
* Now select `UNLOCK ASSEMBLY BUTTON` and click `START GENOME ASSEMBLY`. Now is a good time to go have a coffee. 
    * _ESPECIALLY for Whole-Genome SARS-CoV-2: covering a 30kb genome with up to 200,000 short reads can take a very long time to process._
